...
function fixcolumns($vertical) {
	...
	$vertical.forEach(function ($verticalChild) {
		$verticalHeight[$verticalHeight.length]=removeTiles(Array.prototype.slice.call($verticalChild.querySelectorAll('.tile')));
	});
	$vertical.forEach(function ($verticalChild) {
...
function fixtiles() {
	let $tiles = getAll('.is-ancestor');
	$tiles.forEach(function ($ancestor) {
		$vertical=Array.prototype.slice.call($ancestor.querySelectorAll('.is-vertical'));
		if ($vertical.length > 0) {
			fixcolumns($vertical);
		} else {
			fixcolumns(Array.prototype.slice.call($ancestor.querySelectorAll('.is-parent')));
		}
	});
}
